...
test("pattern occurrence matching preserves exact empty cells from pattern engine motifs", () => {
  const board = { size: 2, cells: [1, 0, 0, 1] };
  const matches = findPatternOccurrences(board, {
    signature: "binary",
    width: 2,
    height: 2,
    cells: ["O", "E", "E", "O"]
  ...
  assert.deepEqual(matches[0].occupiedKeys.sort(), ["0,0", "1,1"]);
});

test("pattern occurrence matching preserves numeric empty cells from legacy motifs", () => {
  const board = { size: 2, cells: [1, 0, 0, 1] };
  const matches = findPatternOccurrences(board, {
    signature: "legacy",
    width: 2,
    height: 2,
    cells: [CELL.REQUIRED, CELL.EMPTY, CELL.EMPTY, CELL.REQUIRED]
  });
