...
function snapshot(size, occupied) {
...
function runSimulation(settings, seed, options = {}) {
  ...
  const interval = Math.max(1, Math.round(options.interval ?? (turns || 1)));
  const checkpoints = [];
  ...
  for (let turn = 1; turn <= turns; turn += 1) {
    ...
    if (options.collectCheckpoints && (turn % interval === 0 || turn === turns)) {
      const last = checkpoints.at(-1);
      if (!last || last.turn !== turn) checkpoints.push({ turn, state: snapshot(size, occupied) });
    }
  }

  return options.collectCheckpoints ? checkpoints : snapshot(size, occupied);
}

export function simulateFinalState(settings, seed) {
  return runSimulation(settings, seed);
}

export function simulateCheckpoints(settings, seed, options = {}) {
  return runSimulation(settings, seed, { ...options, collectCheckpoints: true });
}
