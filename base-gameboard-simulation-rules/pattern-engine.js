  ...
  for (const size of settings.windowSizes) {
    ...
    for (let originY = 0; originY <= board.size - height; originY += 1) {
      for (let originX = 0; originX <= board.size - width; originX += 1) {
        ...
        if (occupiedCount < settings.minOccupiedCount || ratio < settings.minOccupancyRatio || ratio > settings.maxOccupancyRatio) continue;
...
export function extractRegionMotifs(board, options = {}) {
  const settings = { ...defaultPatternSettings(), ...options };
  const maxRegionArea = Math.max(1, Math.round(settings.maxRegionArea));
  const minOccupiedCount = Math.max(1, Math.round(settings.minOccupiedCount));
  ...
  for (let startY = 0; startY < board.size; startY += 1) {
    for (let startX = 0; startX < board.size; startX += 1) {
      ...
      const occupiedCount = component.length;
      const ratio = occupiedCount / (width * height);
      if (occupiedCount < minOccupiedCount || ratio < settings.minOccupancyRatio || ratio > settings.maxOccupancyRatio) continue;
      const candidate = withIdentity({ originX: minX, originY: minY, width, height, cells, occupiedCount }, board, "region");
      if (candidate.externallyAccessible) result.push(candidate);
    }
  }
  return result;
