...
export function crossover(a, b, random) {
  ...
  return {
    ...
    distanceMode: (a.distanceMode ?? 0) * t + (b.distanceMode ?? 0) * (1 - t),
    cohesion: (a.cohesion ?? 0) * t + (b.cohesion ?? 0) * (1 - t),
...
export function mutate(candidate, random, strength = 12) {
  ...
  return {
    ...
    bishop: moves.bishop * 100,
    knight: moves.knight * 100,
    distanceMode: Math.max(0, Math.min(1, (candidate.distanceMode ?? 0) + delta() / 100)),
    cohesion: Math.max(0, Math.min(100, (candidate.cohesion ?? 0) + delta())),
