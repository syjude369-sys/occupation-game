...
function runSimulation(gridCount, playableCells, cellSize, halfGrid) {
  ...
  for (let turn = 0; turn < turns; turn += 1) {
    const protectedKeys = protectedCellKeys(occupied, history, turn, gridCount);
...
function protectedCellKeys(occupied, history, currentTurn, gridCount) {
  ...
  for (let ix = 0; ix < gridCount - 1; ix += 1) {
    for (let iy = 0; iy < gridCount - 1; iy += 1) {
      const keys = [
        cellKey(ix, iy),
        cellKey(ix + 1, iy),
        cellKey(ix, iy + 1),
      ...
      const owner = occupied.get(keys[0]);
      if (owner == null) continue;
      if (keys.every((key) => occupied.get(key) === owner)) {
